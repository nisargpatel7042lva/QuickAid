...
import { Heart, Mic, Phone, Type, Package, ChevronDown, PlayCircle, AlertCircle, Target, Compass, Eye, Mail, Globe, Github } from 'lucide-react';
...
function App() {
  ...
  const emergencyInstructions = {
    ...
    seizure: [
      ...
      "Time the seizure and call emergency if it lasts more than 5 minutes"
    ]
  };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-red-50 via-red-100 to-red-50">
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <h1 className="text-2xl sm:text-3xl font-bold text-gray-800">QuickAid</h1>
            </div>
            <button
              onClick={handleEmergencyCall}
              className="flex items-center space-x-2 bg-red-500 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg hover:bg-red-600 transition-all transform hover:scale-105 shadow-md text-sm sm:text-base"
            >
              <Phone className="h-4 w-4 sm:h-5 sm:w-5" />
              <span className="font-semibold hidden xs:inline">Emergency</span>
      ...
      <main className="container mx-auto px-4 py-6 sm:py-8 flex-grow">
        <div className="max-w-4xl mx-auto space-y-6 sm:space-y-8">
          <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8 border border-red-100">
            <h2 className="text-xl sm:text-2xl font-bold text-gray-800 mb-4 sm:mb-6">What's the Emergency?</h2>
            <div className="flex items-center space-x-3 sm:space-x-4">
              ...
              <div className="flex-1 relative" ref={inputRef}>
                <input
                  ...
                  className="w-full px-4 sm:px-6 py-3 sm:py-4 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-red-500 focus:border-transparent shadow-sm text-base sm:text-lg"
                />
                <Type className="absolute right-4 top-1/2 transform -translate-y-1/2 h-5 w-5 sm:h-6 sm:w-6 text-gray-400" />
                  ...
                  <div className="absolute w-full mt-2 bg-white border border-gray-200 rounded-xl shadow-lg z-50 max-h-60 overflow-y-auto">
                    {suggestions.map((suggestion, index) => (
                      <button
                        ...
                        className="w-full px-4 sm:px-6 py-2 sm:py-3 text-left hover:bg-red-50 transition-colors text-gray-700 flex items-center space-x-2 text-sm sm:text-base"
                      >
          ...
          {inputText && (
            <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8 border border-red-100">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-6 space-y-4 sm:space-y-0">
                <h2 className="text-xl sm:text-2xl font-bold text-gray-800">Emergency Instructions</h2>
                {videoUrl && (
                  <a
                    href={videoUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center justify-center space-x-2 bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600 transition-all transform hover:scale-105 shadow-md text-sm sm:text-base w-full sm:w-auto"
                  >
              ...
              <div className="space-y-3 sm:space-y-4">
                {instructions.map((instruction, index) => (
                  <div
                    key={index}
                    className="flex items-start space-x-3 sm:space-x-4 p-4 sm:p-6 bg-gradient-to-r from-red-50 to-white rounded-xl border border-red-100 shadow-sm hover:shadow-md transition-all"
                  >
                    <div className="flex-shrink-0 w-8 h-8 sm:w-10 sm:h-10 bg-red-500 text-white rounded-xl flex items-center justify-center font-bold text-base sm:text-lg shadow-sm">
                      {index + 1}
                    </div>
                    <div className="flex-1">
                      <p className="text-gray-700 text-base sm:text-lg leading-relaxed">{instruction}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8 border border-red-100">
            <button
              ...
              className="w-full flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 bg-gradient-to-r from-red-50 to-white rounded-xl hover:from-red-100 transition-all shadow-sm"
            >
              <div className="flex items-center space-x-3">
                <div className="bg-red-500 p-2 rounded-lg shadow-sm">
                  <Package className="h-5 w-5 sm:h-6 sm:w-6 text-white" />
                </div>
                <span className="text-lg sm:text-xl font-bold text-gray-800">First Aid Kit Guide</span>
              </div>
              <ChevronDown className={`h-5 w-5 sm:h-6 sm:w-6 text-gray-500 transform transition-transform ${showFirstAidKit ? 'rotate-180' : ''}`} />
            </button>
            
            {showFirstAidKit && (
              <div className="mt-4 sm:mt-6 grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-2">
                {Object.entries(firstAidKit).map(([item, info]) => (
                  <div key={item} className="p-4 sm:p-6 bg-gradient-to-r from-red-50 to-white rounded-xl border border-red-100 shadow-sm hover:shadow-md transition-all">
                    ...
                    <div className="space-y-2 sm:space-y-3 text-gray-600">
                      <p className="font-medium text-sm sm:text-base">{info.description}</p>
                      <p className="text-xs sm:text-sm border-t border-red-100 pt-2">
      ...
      <footer className="bg-white/95 backdrop-blur-sm border-t border-red-100 mt-12">
        <div className="container mx-auto px-4 py-12">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="space-y-4">
              ...
              <p className="text-gray-600 leading-relaxed">
                To empower individuals with instant access to life-saving first aid knowledge, making emergency response accessible to everyone, anywhere, anytime.
              </p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <p className="text-gray-600 leading-relaxed">
                To reduce response time in emergencies through quick, reliable guidance and create a world where everyone is prepared to handle medical emergencies confidently.
              </p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <p className="text-gray-600 leading-relaxed">
                To be the most trusted digital first aid assistant globally, saving lives through technology and empowering communities with essential emergency response knowledge.
              </p>
            </div>
          </div>
          ...
          <div className="mt-12 pt-8 border-t border-red-100">
            <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
              <div className="flex items-center space-x-3">
                ...
                <div className="text-gray-800">
                  <h2 className="font-bold text-lg">QuickAid</h2>
                  <p className="text-sm text-gray-600">Your Digital First Aid Assistant</p>
                </div>
              </div>

              <div className="flex items-center space-x-6">
                <a href="mailto:contact@quickaid.com" className="text-gray-600 hover:text-red-500 transition-colors">
                  <Mail className="h-5 w-5" />
                </a>
                <a href="https://quickaid.com" target="_blank" rel="noopener noreferrer" className="text-gray-600 hover:text-red-500 transition-colors">
                  <Globe className="h-5 w-5" />
                </a>
                <a href="https://github.com/quickaid" target="_blank" rel="noopener noreferrer" className="text-gray-600 hover:text-red-500 transition-colors">
            ...
            <div className="mt-8 text-center text-sm text-gray-600">
              <p>© {new Date().getFullYear()} QuickAid. All rights reserved.</p>
              <p className="mt-2">
                Disclaimer: This app provides first aid guidance but is not a substitute for professional medical care.
